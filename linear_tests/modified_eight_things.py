...
sizeItemRepLayer = 12 #ize of first item hidden layer
sizeContextRepLayer = 8  #size of first context hidden layer
sizeSecondLayer = 16  #size of second hidden layer
...
#multiplied weights
W1_item = tf.Variable(tf.random_uniform([sizeItemRepLayer,nSizeofItemInput],-0.05,0.05)) #weights from item input to item hidden layer1
B1_item = tf.Variable(tf.random_uniform([sizeItemRepLayer,1],-0.05,0.05)) #bias from item input to item hidden layer1

W1_context = tf.Variable(tf.random_uniform([sizeContextRepLayer,nSizeofContextInput],-0.05,0.05)) #weights from context input to context hidden layer1
B1_context = tf.Variable(tf.random_uniform([sizeContextRepLayer,1],-0.05,0.05)) #bias from context input to context hidden layer1

W2_item = tf.Variable(tf.random_uniform([sizeSecondLayer,sizeItemRepLayer],-0.05,0.05))#weights from item hidden layer1 to layer2
B2_item = tf.Variable(tf.random_uniform([sizeSecondLayer,1],-0.05,0.05)) #bias from item hidden layer1 to layer2

W2_context = tf.Variable(tf.random_uniform([sizeSecondLayer,sizeContextRepLayer],-0.05,0.05)) #weights from context hidden layer1 to layer2
B2_context = tf.Variable(tf.random_uniform([sizeSecondLayer,1],-0.05,0.05)) #bias from context hidden layer1 to layer2

W3 = tf.Variable(tf.random_uniform([nSizeofOutput,sizeSecondLayer],-0.05,0.05)) #weights from context hidden layer1 to layer2
B3 = tf.Variable(tf.random_uniform([nSizeofOutput,1],-0.05,0.05)) #bias from context hidden layer1 to layer2
...
secondhidden = ((tf.matmul(W2_item,itemhidden) + B2_item) + (tf.matmul(W2_context,contexthidden) + B2_context))
output = tf.nn.sigmoid(tf.matmul(W3, secondhidden) + B3)
